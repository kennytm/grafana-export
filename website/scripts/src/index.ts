...
try {
  ...
  class Exporter {
    ...
    constructor() {
      ...
      this.exportButton = $('<button style="font-weight:bold">')
        .text(L.export)
        .one("click", () => this.startExport());
      this.expandAllButton = $("<button>")
        .text(L.expandAllPanels)
        .on("click", () => dashboard.expandRows());
      // note: order like [------- (Cancel) (Export)] on macOS,
      //              and [--- (Export) (Cancel) ---] everywhere else.
      const flexStyle = /^(Mac|iP)/.test(navigator.platform)
        ? "flex-direction:row-reverse"
        : "justify-content:center";
      this.elem = $('<div class="__dexport_hint">').append(
        $("<p>")
          .text(L.exportTitle)
          .append($("<strong>").text(dashboard.title)),
        $("<p>").text(L.exportSubtitle).append(this.expandAllButton),
        $('<p style="font-size:.8em">').append(
          this.loadProgress,
          this.loadProgressText
        ),
        $(`<p style="display:flex;${flexStyle}">`).append(
          this.exportButton,
          $("<button>")
            .text(L.cancel)
            .on("click", () => this.closeUI())
        )
      );
    ...
    updateProgress(): void {
      ...
      if (this.exportState === ExportState.idle) {
        ...
        this.loadProgressText.text(
          `${L.loadingPanels}: ${percent}% (${ready}/${total})`
        );
        ...
        if (ready >= total) {
          ...
          this.exportButton
            .prop("disabled", false)
            .text(L.exportImmediately)
            .one("click", () => this.doExport());
        }
      }
    }
    ...
    doExport(): void {
      ...
      clone.id = null;
      clone.uid = null;
      clone.title = `${title} (Export at ${timestamp})`;
      const snapshot = {
        meta: {
          isSnapshot: true,
          type: "snapshot",
